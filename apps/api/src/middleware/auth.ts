...
export interface UserClaims {
  ...
  name?: string | null;
  picture?: string;
  roles: string[];             // raw string roles from token
  role?: $Enums.Role;          // derived Prisma enum (first match)
  orgRoles?: $Enums.OrgRole[]; // optional: derived Org roles (all matches)
  orgRole?: $Enums.OrgRole;    // optional: first org-role match
  [k: string]: any;
}


// --- JWKS Verifier Setup (Cached) ---
let jwks: ReturnType<typeof createRemoteJWKSet> | undefined = undefined;
if (JWKS_URI) {
  try {
    // --- FIX: Removed 'cache' and 'cooldownDuration' ---
...
/**
 * Extract a normalized user object from the JWT payload.
 */
function toUser(payload: JWTPayload): UserClaims {
  const sub = typeof payload.sub === 'string' ? payload.sub : undefined;

  // robust email extraction: email or emails[0]
  const email =
  ...
  // 1) Collect "roles" from common claim shapes
  ...
  let roleStrings: string[] =
    ...
    toStrArray((payload as any).role) ||      // some IdPs put a single string here
    toStrArray((payload as any).scope);       // space-delimited scopes

  if (!roleStrings.length) {
    for (const [k, v] of Object.entries(payload)) {
      if (/roles?$/i.test(k)) {               // any key ending with role(s)
  ...
  // 2) Collect "org roles" from common org/tenant shapes (optional)
  ...
  // 3) Map strings -> Prisma enums
  const ROLE_MAP: Record<string, $Enums.Role> = {
    admin: 'ADMIN',
    lawyer: 'LAWYER',
    attorney: 'LAWYER',     // synonym
    paralegal: 'PARALEGAL',
    'para-legal': 'PARALEGAL',
    client: 'CLIENT',
    customer: 'CLIENT',     // synonym
  };
...
function getBearer(req: Request): string | null {
  const h = req.header('Authorization'); // Use .header() function
